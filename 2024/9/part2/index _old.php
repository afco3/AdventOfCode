...
while (true) {
    ...
    //     dump($file);
    //     dump($freeSpaceKey);
    //     dump($partition[$freeSpaceKey]);
    //     break;
    // }

    if (is_null($freeSpaceKey) || $freeSpaceKey > $fileKey) continue;
    // displayPartition($partition);
    ...
    $tmpPartition[$freeSpaceKey+1]['size'] = $tmpPartition[$freeSpaceKey+1]['size'] - $file['size'];
    // dump($partition);
    $partition[$fileKey]['value'] = ".";
    $tmpPartition[$fileKey]['value'] = ".";

    $partition = array_values($partition);
    $tmpPartition = array_values($tmpPartition);
